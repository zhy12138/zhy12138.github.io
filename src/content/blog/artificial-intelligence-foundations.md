...
- SegNet 在编码—解码卷积网络中使用跳跃连接，把编码阶段保留的低层信息带回解码阶段。
- PSPNet 在跳跃连接基础上加入空间金字塔池化，以不同尺度汇聚上下文信息。
...
- CPM 属于自顶向下方法：先检测人的边界框，用 VGG 提取特征，再由多阶段 CNN 反复细化关键点热图，较大感受野有利于逐步修正估计。
- OpenPose 属于自底向上方法：整图提取特征后同时预测关键点及其连接，再把关键点分配给不同的人；固定推理规模是优势，逐像素解析热图和分配连接是开销来源。
- PPN 把 YOLO 式检测与 OpenPose 结合，把身体连接及其边界框视作检测目标，再用贪心方法把连接组合成人体姿态。
