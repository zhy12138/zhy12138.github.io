...
- 原笔记来源：[算分.md](https://github.com/zhy12138/class_notes/blob/main/%E7%AE%97%E5%88%86/%E7%AE%97%E5%88%86.md)
...
回溯主要剪去违反约束、已无法扩展成可行解的分支；分支限界还使用目标函数界，剪去即使补全也不可能优于当前最好解的分支。最大化问题常维护安全上界，最小化问题常维护安全下界。界可以不够紧，但不能错误排除最优解。

搜索结点通常保存部分决策、当前目标、剩余候选和界。深度优先占用空间少，且可能较早找到可行解以收紧界；最佳优先按界扩展，更可能快速接近最优值，但活结点优先队列可能很大。

### 最大团、TSP 与混合布尔优化

最大团可用“当前团大小 + 剩余候选数”作上界，图着色数还能给出更紧上界。TSP 可用当前路径长度加未连接顶点所需最短边构造下界。混合布尔凸优化固定离散变量后求凸子问题界，再对未定布尔变量分支；界函数质量直接决定实际搜索规模。
...
在单链表中访问第 $i$ 个元素的代价为 $i$，交换一对相邻元素的代价为 $O(1)$。MTF（Move-To-Front）每次访问元素 $x$ 后，用相邻交换把它移动到表头。

比较 MTF 与任意策略 $A$，定义势函数：

$$
\Phi(T)=2\sum_{i<j}[i,j\text{ 在 }A\text{ 与 MTF 中顺序不同}]
$$

设访问前 $x$ 在 MTF 中排第 $k$，在 $A$ 中排第 $i$：

1. MTF 访问和前移的实际代价为 $k+(k-1)$。
2. 在 $A$ 尚未交换元素时，前移 $x$ 引起的势能变化至多为 $4\min(k-1,i-1)-2(k-1)$。
3. 因而这一步的平摊代价
   $$
   \hat c\leq k+(k-1)+4\min(k-1,i-1)-2(k-1)\leq4i.
   $$
4. 若 $A$ 随后做 $j$ 次相邻交换，势能至多增加 $2j$，仍不超过 $A$ 交换代价的 4 倍。

所以对任意访问序列，MTF 的总代价不超过任意其他策略总代价的 4 倍。这里比较的是整段序列的竞争保证，不是说每一次访问都逐次快于其他策略。

三种方法结论等价但视角不同：聚集分析直接估计最坏操作序列总成本；记账法给便宜操作多收费并储存信用；势能法把信用统一写成状态函数。证明必须保证任意前缀信用不为负，或满足 $\Phi(D_i)\geq\Phi(D_0)$，否则平摊总和可能低估真实总成本。
...
单纯形从一个基本可行解沿可行域多面体的边移动。最小化问题中，负检验数给出可改进的换入方向；比值检验保证变量增加时仍保持非负，并确定换出变量。若换入列没有限制其增长的正系数，目标无界。退化时步长可能为 0，Bland 规则用于避免基之间循环。
...
弱对偶给出可验证的目标界：任意原、对偶可行解都满足固定大小关系。若找到目标值相同的一对可行解，就能直接证明双方最优。互补松弛把这种相等关系拆成逐变量条件，也常用于从一侧最优解恢复另一侧。
...
### Ford-Fulkerson、Edmonds-Karp 与 Dinic
...
余量网络的反向边表示可以撤销既有流量，并非原网络中的真实通道。Ford-Fulkerson 在整数容量时每次至少增广 1，因而有限终止；任意实数容量下，随意选路可能不终止。Edmonds-Karp 固定选择边数最少的增广路，Dinic 则在同一分层图中一次求阻塞流。

![Ford-Fulkerson 在无理容量网络中的不终止构造](/blog/design-and-analysis-of-algorithms/ford-fulkerson-irrational-capacity.png)

图中取 $r=(\sqrt5-1)/2$，满足 $1-r=r^2$。若每轮都按特定增广路选择，可让增广量依次成为 $r^n$，不断趋近于 0 而不在有限步达到最大流。这说明“存在增广路”不足以保证任意选路策略有限终止。
...
建模时要先明确“哪些边必须满流、割断一条边表示放弃什么”。带下界流先预送每条边的下界，再把各点不平衡量转成需求；增设超级源和超级汇后，检查附加边是否全部满流。最大闭包类问题用无限容量依赖边保证选择一个项目时必须选择其前置项目。
...
![选择问题的比较次数上界与空间对照](/blog/design-and-analysis-of-algorithms/selection-comparison-bounds.png)

找最大和最小的下界可用对手状态证明。把元素标为 `N`（未比较）、`W`（只赢过）、`L`（只输过）、`WL`（既赢又输）；算法结束时必须留下一个 `W`、一个 `L`，其余全为 `WL`。对手总选择让一次比较新增的信息单位尽量少，从而迫使算法至少进行 $\lceil3n/2\rceil-2$ 次比较。

![找最大和最小的对手状态转移表](/blog/design-and-analysis-of-algorithms/selection-adversary-state-table.png)
...
证明一个新判定问题 $B$ 为 NP 完全通常分两步：先给出多项式时间验证器说明 $B\in NP$；再从已知 NP 完全问题 $A$ 构造多项式变换 $A\le_p B$。规约方向不能写反，从 $B$ 规约到已知难问题只说明 $B$ 不比它更难。

规约还要证明原实例为 YES 当且仅当构造后的实例为 YES，并说明构造规模与时间为多项式。优化问题通常先增加目标阈值，改写成判定问题再讨论 NP 完全性。
...
近似证明必须把算法值与同一实例的最优值联系起来。分析通常先找一个容易计算的最优值下界，再证明算法代价不超过该下界的常数倍。PTAS 对每个固定 $\varepsilon$ 都在输入规模上为多项式时间，但指数可依赖 $1/\varepsilon$；FPTAS 还要求对输入规模和 $1/\varepsilon$ 都为多项式时间。
...
![TSP 的 MST 两倍近似紧例](/blog/design-and-analysis-of-algorithms/tsp-mst-tight-example.png)

MST 法先求最小生成树，再把树边加倍得到欧拉回路，最后利用三角形不等式跳过重复顶点，因此算法值小于 $2OPT$。图示构造满足 $OPT(I)=2n$、$MST(I)=4n-2$，近似比达到 $2-1/n$，说明仅靠这套分析不能把保证改进到 2 以下。
...
### Las Vegas、Monte Carlo 与复杂度类

- 拉斯维加斯算法：结果总正确，运行时间随机；有效时要求期望多项式时间。
- 蒙特卡洛算法：运行时间多项式，结果有概率出错。
- RP：单侧错误，弃真型；co-RP：单侧错误，取伪型；BPP：双侧错误。

Las Vegas 的随机性只影响运行时间，输出始终正确；Monte Carlo 保证多项式时间但允许错误。若单次独立试验错误概率小于 $1/2$，重复并多数表决可指数降低错误率；使用概率放大时必须保证各次随机选择独立。
...
竞争分析比较任意请求序列上的在线代价和预知完整未来的离线最优代价。加性常数 $c$ 不能随序列长度增长；竞争比描述最坏序列保证，不等于平均实际性能。证明上界常设计势函数，证明下界则由对手根据算法当前状态生成请求。
