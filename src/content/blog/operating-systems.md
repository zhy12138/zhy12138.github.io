...
- 补充笔记来源：[优缺点.md](https://github.com/zhy12138/class_notes/blob/main/%E6%93%8D%E7%BB%9F/%E4%BC%98%E7%BC%BA%E7%82%B9.md)
...
- **加载时映射**：装入程序时直接改写地址，运行时没有额外翻译开销，硬件简单；但程序地址难以改变，也难以支持动态链接等需求。
- **硬件实时翻译**：每次访问由硬件执行 `Base + virtual_address`，地址空间更灵活、可扩展；代价是运行时多一次检查和加法，并需要专用硬件。
...
倒置页表改为“每个物理页框一个表项”，页表大小只随物理内存增长，不随虚拟地址空间增长；但查询需要按“进程标识 + 虚拟页号”做散列，访问模式缺少普通页表的局部性，冲突处理也更复杂。
...
- **物理索引缓存**：一个物理块只会映射到一个位置，上下文切换通常无需刷新；但必须先完成 TLB 翻译，TLB 位于命中关键路径。
- **虚拟索引缓存**：可与地址翻译并行甚至绕开 TLB，命中更快；但同一物理块可能因不同虚拟地址产生多个副本，上下文切换还可能需要刷新或额外标识。

写策略也影响命中路径：

- **写穿透**同时更新 cache 与下层内存，状态简单，读缺失无需先写回脏块；缺点是每次写都触发慢速下层写入，通常需要写缓冲。
- **写回**只更新 cache 并标记 dirty，替换时才写回；重复写不会反复访问 DRAM，但控制更复杂，读缺失若替换脏块会额外产生一次写回。
